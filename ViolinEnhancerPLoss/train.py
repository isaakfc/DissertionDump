...
def compute_single_embedding(image_array):
    ...
    temp_filepath = os.path.join(temp_folder, "temp_spectrograms.png")
    ...
    emb = openl3.get_image_embedding(loaded_image_array, model=model, verbose=0)
...
@tf.function
def WGAN_GP_train_g_step(real_x, noisy_x, discriminator, generator, generator_optimizer, gamma=0.5, zeta=0.25):
    ###################################
    # Train G
    ###################################
    with tf.GradientTape() as g_tape:
        ...
        # Combined loss
        total_g_loss = -tf.reduce_mean(fake_pred) + gamma * recon_loss + zeta * perceptual_loss
